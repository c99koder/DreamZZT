...
void ZZTOOP::exec(std::string text) {
	...
	if(words[0] == "go") {
		...
		if(color!=-1) {
			...
			if(words.size() < 3)
				words.push_back(std::string("empty"));
		}
		if(str_to_obj(words[2])==-1) {
			...
			o=get
			  (str_to_direction(words[1]));
			if(o!=NULL && o->flags() & F_PUSHABLE)
				o->move(str_to_direction(words[1]),true);
			...
			if(o->bg()>7)
				o->setBg(o->bg() - 8);
			remove_from_board(currentbrd,get
			                  (str_to_direction(words[1])));
			put(o);
		}
		goagain=1;
	...
	/*else if(words[0] == "teleport") {
		...
		if(words[1] == "gems") {
			if(words.size() > 3 && world.gems<atoi(words[2].c_str())) {
				zzt_goto(words[3]);
			} else
				take_gems(atoi(words[2].c_str()));
		}
		if(words[1] == "torch" || words[1] == "torches") {
			if(words.size() > 3 && world.torches<atoi(words[2].c_str())) {
				zzt_goto(words[3]);
			} else
				take_torch(atoi(words[2].c_str()));
		}
		if(words[1] == "ammo") {
			if(words.size() > 3 && world.ammo<atoi(words[2].c_str())) {
				zzt_goto(words[3]);
			} else
				take_ammo(atoi(words[2].c_str()));
		}
		if(words[1] == "score") {
			if(words.size() > 3 && world.score<atoi(words[2].c_str())) {
				zzt_goto(words[3]);
			} else
				take_score(atoi(words[2].c_str()));
		}
		if(words[1] == "health") {
			if(words.size() > 3 && world.health<atoi(words[2].c_str())) {
				zzt_goto(words[3]);
			} else
				take_health(atoi(words[2].c_str()));
		}
		if(words[1] == "time") {
			if(currentbrd->time<atoi(words[2].c_str())) {
				zzt_goto(words[3]);
			} else
				take_time(atoi(words[2].c_str()));
		}
		goagain=1;
	} else if(words[0] == "die") {
		task_kill(this);

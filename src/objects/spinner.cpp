...
int spin_cw(struct object *me, struct object *them) {
  if(!(them->flags&F_PUSHABLE)) return 0;
    ...
    for(x=0;x<3;x++) {
      if(moved[x][y]==0 && !(x==1 && y==1)) {
        if(currentbrd->board[me->x+(x-1)][me->y+(y-1)].obj!=NULL) {
          if(me->type==ZZT_SPINNER_CW) spin_cw(me,currentbrd->board[me->x+(x-1)][me->y+(y-1)].obj);
          else if(me->type==ZZT_SPINNER_CCW) spin_ccw(me,currentbrd->board[me->x+(x-1)][me->y+(y-1)].obj);
        }
      }
    }
  }
